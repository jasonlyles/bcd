...
class AuthenticationsController < ApplicationController
  before_action :authenticate_user!, except: %i[create clear_authentications failure]
  skip_before_action :find_cart, only: %i[create failure]

  def failure
    flash[:notice] = if params['message'] == 'invalid_credentials' && params['strategy'] && !params['strategy'].blank?
                       "Sorry, the credentials you provided were rejected by #{params['strategy'].capitalize}. Please try again."
                     else
                       'Sorry, the authentication failed. Please try again. If it continues to fail, you can still check out as a guest.'
                     end
