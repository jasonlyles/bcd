class Element < ActiveRecord::Base
  ...
  def update_from_rebrickable(part_id, color_id)
    ...
    if data['part_img_url'].present?
      self.original_image_url = data['part_img_url']
      self.part.check_rebrickable = false
      self.part.save!
    end
  end
  ...
  def update_from_bricklink(part_id, color_id)
    ...
    if data['data'].present? && data['data']['thumbnail_url'].present?
      self.original_image_url = "https:#{data['data']['thumbnail_url']}"
      self.part.check_bricklink = false
      self.part.save!
    end
  end
  ...
  def self.get_part_and_color_name_string(part_key)
    part, color = get_part_and_color(part_key)
    if part && color
      "#{color.bl_name} #{part.name}"
    else
      "Either part or color missing for #{part_key}"
    end
  end
  ...
  def self.find_or_create_via_external(part_key)
    ...
    begin
      ...
      binding.pry # TODO: Remove this when I've got figured out what to do here.
    end
  ...
  def store_image
    ...
    if original_image_url
      ...
      begin
        ...
        logger.error "There was a problem getting an image for #{original_image_url}"
      end
