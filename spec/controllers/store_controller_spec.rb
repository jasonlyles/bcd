...
describe StoreController do
  ...
  describe 'GET cart' do
    ...
    it 'should add errant items to flash notice' do
      ...
      @base_product = FactoryBot.create(:product, category_id: @category.id, subcategory_id: @subcategory.id, product_type_id: @product_type.id, name: 'Winter Village Flak Cannon', quantity: 1, product_code: 'WV009')
      @product = FactoryBot.create(:product, category_id: @category.id, subcategory_id: @subcategory.id, product_type_id: @product_type2.id, name: 'Winter Village Flak Cannon Model', quantity: 3, product_code: 'WV009M')
      @cart = FactoryBot.create(:cart)
      @cart_item = FactoryBot.create(:cart_item, product_id: @product.id, cart_id: @cart.id, quantity: 5)
  ...
  describe 'add_to_cart' do
    ...
    it 'should not add to cart if the item is already in the cart' do
      @product = FactoryBot.create(:product, product_type_id: @product_type.id)
      @cart = Cart.new
      request.env['HTTP_REFERER'] = '/'
      post :add_to_cart, params: { product_code: @product.product_code }
      post :add_to_cart, params: { product_code: @product.product_code }
  ...
  describe 'remove_item_from_cart' do
    ...
    it "should redirect back with notice if item couldn't be removed from the cart" do
      @product = FactoryBot.create(:product)
      @cart = FactoryBot.create(:cart)
      @cart.cart_items << FactoryBot.create(:cart_item, cart_id: @cart.id)
      request.env['HTTP_REFERER'] = '/'
  ...
  describe 'update_item_in_cart' do
    it 'should update an item in the cart' do
      ...
      @base_product = FactoryBot.create(:product, category_id: @category.id, subcategory_id: @subcategory.id, quantity: 1, product_type_id: @product_type.id, product_code: 'XX111')
      @product = FactoryBot.create(:product, category_id: @category.id, subcategory_id: @subcategory.id, quantity: 30, product_type_id: @product_type2.id, product_code: 'XX111M', name: 'fake product')
      @cart = FactoryBot.create(:cart)
      session[:cart_id] = @cart.id
      @cart_item = FactoryBot.create(:cart_item, cart_id: @cart.id, product_id: @product.id)
      @cart.cart_items << @cart_item
  ...
  describe 'checkout' do
    it 'should redirect to guest_registration if there is no current_customer' do
      ...
      @cart = FactoryBot.create(:cart, user_id: nil)
      session[:cart_id] = @cart.id
    ...
    it 'should redirect to cart if a user has already purchased a set of instructions and has downloaded, but downloads remaining is > 0' do
      @user = FactoryBot.create(:user)
      @product = FactoryBot.create(:product, product_type_id: @product_type.id)
      @download = FactoryBot.create(:download, product_id: @product.id, user_id: @user.id, remaining: 1)
      @order = FactoryBot.create(:order_with_line_items)
      @cart = FactoryBot.create(:cart_with_cart_items, user_id: @user.id)
      request.env['HTTP_REFERER'] = '/'
      sign_in @user
    ...
    it 'should not redirect to cart if a user has already purchased a set of instructions, but has 0 downloads remaining' do
      @user = FactoryBot.create(:user)
      @product = FactoryBot.create(:product, product_type_id: @product_type.id)
      @download = FactoryBot.create(:download, product_id: @product.id, user_id: @user.id, remaining: 0)
      @order = FactoryBot.create(:order_with_line_items)
      @cart = FactoryBot.create(:cart_with_cart_items, user_id: @user.id)
      request.env['HTTP_REFERER'] = '/'
      sign_in @user
    ...
    it "should not redirect to cart if a user has added a physical product to their cart that they've purchased before" do
      ...
      @line_item = FactoryBot.create(:line_item, order_id: @order.id, product_id: @product2.id)
      @cart = FactoryBot.create(:cart_with_cart_items, user_id: @user.id)
    ...
    it 'should show the flash notice notifying user of bad quantities for physical items' do
      @product_type2 = FactoryBot.create(:product_type, name: 'Models', digital_product: false)
      @user = FactoryBot.create(:user, tos_accepted: true)
      @base_product = FactoryBot.create(:product, category_id: @category.id, subcategory_id: @subcategory.id, product_type_id: @product_type.id, name: 'Winter Village Flak Cannon', quantity: 1, product_code: 'WV009')
      @product = FactoryBot.create(:product, category_id: @category.id, subcategory_id: @subcategory.id, product_type_id: @product_type2.id, name: 'Winter Village Flak Cannon Model', quantity: 3, product_code: 'WV009M')
      @cart = FactoryBot.create(:cart)
      @cart_item = FactoryBot.create(:cart_item, product_id: @product.id, cart_id: @cart.id, quantity: 5)
  ...
  describe 'enter_address' do
    context "current_user & current_user has orders & address hasn't already been submitted" do
      it 'should look at an existing order to get the address used on a previous order' do
        @user = FactoryBot.create(:user)
        FactoryBot.create(:product)
        FactoryBot.create(:order, address_street1: '123 Fake St.', address_state: 'VA')
  ...
  describe 'validate_street_address' do
    context 'if an address is submitted via form' do
      it 'should use the address to create a valid Order hash to save to session' do
        post :validate_street_address, params: { order: { address_submission_method: 'form', address_state: 'HI' } }

        expect(session['address_submitted']['address_state']).to eq('HI')
      end
  ...
  def setup_products
    @category2 = FactoryBot.create(:category, name: 'Random Stuff')
    @category3 = FactoryBot.create(:category, name: 'Alternative Builds')
    @subcategory2 = FactoryBot.create(:subcategory, category_id: 2, code: 'RS')
    @subcategory3 = FactoryBot.create(:subcategory, category_id: @category3.id, code: 'XV')
    @product1 = FactoryBot.create(:product)
    @product2 = FactoryBot.create(:product, product_code: 'XX001', name: 'Ninja Fortress', price: 5)
    @product3 = FactoryBot.create(:product, product_code: 'XX002', name: 'Giant Thing', ready_for_public: 'f', price: 5)
    @product4 = FactoryBot.create(:product, product_code: 'XX003', name: 'Gangsta Hideout', category_id: 2, price: 3)
    @product5 = FactoryBot.create(:product, product_code: 'XV001', name: 'PeeWees Playhouse', category_id: @category3.id, price: 3, alternative_build: 't')
    @product6 = FactoryBot.create(:product, product_code: 'XX004', name: 'Fruitcake Palace', price: 0, free: 't')
  end
end
