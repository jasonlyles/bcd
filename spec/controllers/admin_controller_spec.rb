...
describe AdminController do
  ...
  describe 'gift_instructions' do
    it 'should create a new order with status = GIFT if could not find an order with status = GIFT' do
      ...
      expect(-> { post :gift_instructions, format: :json, gift: { 'user_id' => user.id, 'product_id' => product.id } }).to change { Order.count }.by(1)
    ...
    it 'should destroy the line_item for an order if the user owns the product via gifting' do
      ...
      expect(-> { post :gift_instructions, format: :json, gift: { 'user_id' => user.id, 'product_id' => product.id } }).to change { LineItem.count }.by(-1)
    end
  ...
  describe 'change_user_status' do
    it "should change user's status" do
      @user = FactoryGirl.create(:user, account_status: 'active')
      sign_in @radmin
      xhr :get, :change_user_status, email: @user.email, user: { email: @user.email, account_status: 'cancelled' }, format: :js
      @user = User.find(@user.id)

      expect(@user.account_status).to eq('cancelled')
    end
  end
