...
describe RegistrationsController do
  describe 'create' do
    ...
    context 'can find user based on email passed in' do
      context 'user is saved as a guest' do
        it 'should set user account status to active and make a call to save authentication' do
          @user = FactoryGirl.create(:user, account_status: 'guest')
  ...
  describe 'destroy' do
    it 'should cancel an existing registration, not destroy it' do
      @user = User.create!(email: 'charles@mcwoofay.net', password: 'password', account_status: 'active', tos_accepted: true)
      request.env['devise.mapping'] = Devise.mappings[:user]
      sign_in @user
      get :destroy
      @user = User.find(@user.id)
