...
describe Cart do
  ...
  describe 'add_product' do
    it 'should add product to the cart' do
      ...
      expect { @cart.add_product(@product) }.to change(CartItem, :count).from(0).to(1)
    end
  ...
  describe 'remove_product' do
    it 'should remove product from the cart' do
      ...
      expect { @cart.remove_product(@cart_item.id) }.to change(CartItem, :count).from(2).to(1)
    end
  end
  ...
  it 'should delete related cart_items when it gets deleted' do
    ...
    expect { cart.destroy }.to change(CartItem, :count).from(1).to(0)
  end
  ...
  describe 'update_product_quantity' do
    it 'should remove item from cart if the quantity is brought down to 0' do
      ...
      expect { @cart.update_product_quantity(1, 0) }.to change(CartItem, :count).from(1).to(0)
    end
    ...
    it 'should update the cart_item and update the quantity if quantity is > 0' do
      ...
      expect(@cart.cart_items[0].quantity).to eq(2)
      expect { @cart.update_product_quantity(1, 3) }.to_not change(CartItem, :count)
      @cart.reload
