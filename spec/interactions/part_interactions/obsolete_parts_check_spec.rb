  ...
  describe "run" do
    context 'part is not marked obsolete' do
      it 'should not create any BackendNotification records' do
        ...
        expect(lambda{PartInteractions::ObsoletePartsCheck.run({})}).not_to change(BackendNotification, :count)
      end
    end
    ...
    context 'part is now marked obsolete' do
      it 'should create a BackendNotification record and mark the part obsolete' do
        ...
        expect(lambda{PartInteractions::ObsoletePartsCheck.run({})}).to change(BackendNotification, :count).from(0).to(1)
