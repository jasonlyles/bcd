  ...
  describe 'run' do
    ...
    context 'old and new parts are found by name' do
      it 'should succeed, set affected_parts_lists_ids and update relevant lots' do
        ...
        parts_list1 = FactoryBot.create(:parts_list, name: 'Test1', product: product, original_filename: 'test.xml', bricklink_xml: File.read(File.join(Rails.root, 'spec', 'support', 'parts_lists', 'test.xml')))
        parts_list2 = FactoryBot.create(:parts_list, name: 'Test2', product: product, original_filename: 'test.xml', bricklink_xml: File.read(File.join(Rails.root, 'spec', 'support', 'parts_lists', 'test.xml')))
        parts_list3 = FactoryBot.create(:parts_list, name: 'Test3', product: product, original_filename: 'test.xml', bricklink_xml: File.read(File.join(Rails.root, 'spec', 'support', 'parts_lists', 'test.xml')))
