...
describe PartsListInteractions::CreatePartsList do
  before do
    ...
    color1 = FactoryGirl.create(:color, ldraw_id: '4', bl_id: '5', bl_name: 'Red')
    color2 = FactoryGirl.create(:color, ldraw_id: '71', bl_id: '86', bl_name: 'Light Bluish Gray')
    color3 = FactoryGirl.create(:color, ldraw_id: '40', bl_id: '13', bl_name: 'Trans-Black')
  ...
  describe 'run' do
    context 'with bricklink xml' do
      it 'should update the parts list and create lots associated with the parts list' do
        parts_list = FactoryGirl.create(:parts_list, name: 'Test', product_id: 1, bricklink_xml: File.read(File.join(Rails.root, 'spec', 'support', 'parts_lists', 'test.xml')))
    ...
    context 'with ldr' do
      it 'should update the parts list and create lots associated with the parts list' do
        parts_list = FactoryGirl.create(:parts_list, name: 'Test', product_id: 1, ldr: File.read(File.join(Rails.root, 'spec', 'support', 'parts_lists', 'test.ldr')))
        expect(parts_list.lots.count).to eq(0)
        expect(parts_list.parts).to be_nil
        expect_any_instance_of(Element).not_to receive(:update_from_rebrickable)
        allow_any_instance_of(ImageUploader).to receive(:present?).and_return(true)
