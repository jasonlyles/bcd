...
describe Etsy::Api::AccessToken do
  describe 'self.store' do
    it 'should create a new token' do
      time = Time.now + 1.hour
      expect {
        Etsy::Api::AccessToken.store('access_token', 'refresh_token', time)
    ...
    it 'should update an existing token' do
      bot = FactoryBot.create(:backend_oauth_token, provider: 'etsy', access_token: 'old', refresh_token: 'even_older', expires_at: Time.now - 1.hour)
      time = Time.now + 1.hour
