...
create_bucket() {
    ...
    # Add the following option to the s3api command below if the region is other than us-east-1 (Stupid AWS CLI bug)
...
pg_dump_database() {
    pg_dump --no-owner --no-privileges --clean --if-exists --quote-all-identifiers "$DATABASE_NAME"
}
...
upload_to_bucket() {
    ...
    s3 cp - "s3://$S3_BUCKET_NAME/$(date +%Y/%m/%d/$DATABASE_NAME-backup-%H-%M-%S.sql.gz)"
}
