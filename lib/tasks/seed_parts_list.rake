namespace :db do
  task seed_colors: :environment do
    ...
    puts "There are now #{PartsList::Color.count} rows in the colors table"
  end
  ...
  task seed_parts: :environment do
    ...
    csv = CSV.parse(csv_text, headers: false, encoding: 'ISO-8859-1')
    csv.each do |row|
      p = PartsList::Part.new
      p.bl_id = row[0]
      p.ldraw_id = row[1]
      p.name = row[2]
      p.save!
    end
    puts "There are now #{PartsList::Part.count} rows in the parts table"
  end
end
